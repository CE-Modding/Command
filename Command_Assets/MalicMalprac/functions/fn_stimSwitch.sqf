...
switch (_ammo) do {
    ...
    case "CMD_MedicalDart_Painkiller": {
    ...
    case "CMD_MedicalDart_Blood": {
        if (_medic == _patient) then {
            [_patient, "activity", "%1 shot themself with Blood", [[_patient] call ace_common_fnc_getName]] call ace_medical_treatment_fnc_addToLog;
            // [_medic, _patient, _selection, "BloodIV", _medic, "ACE_bloodIV"] call kat_circulation_fnc_applyIV;
            [_patient, _selection] call kat_circulation_fnc_applyIV;
        } else {
            [_patient, "activity", "%1 shot %2 with Blood", [[_medic] call ace_common_fnc_getName, [_patient] call ace_common_fnc_getName]] call ace_medical_treatment_fnc_addToLog;
            // [_medic, _patient, _selection, "BloodIV", _medic, "ACE_bloodIV"] call kat_circulation_fnc_applyIV;
            [_patient, _selection] call kat_circulation_fnc_applyIV;
        };
    };
